...
class Dealers_model extends CI_Model {
	...
	public function get_row_Dealers_by_title_status($select, $dealer) {
		if (! empty ( $select )) {
			$this->r_db->select ( $select );
		}
		$this->r_db->where ( 'd_title', $dealer );
		//$this->r_db->where ( 'd_coupon IS NOT NULL', null, false);
		$this->r_db->where ( 'd_coupon !=', '');
